...
// ============================================================
// ФИНАЛЬНЫЙ КОМИКС — ИСПРАВЛЕННАЯ ВЕРСИЯ
// ============================================================
...
function playFinalComicSound(panelNumber) {
  ...
  // Исправленная логика звуков для финального комикса
  if (panelNumber === 1) {
    // Только 1-й кадр — звук заклинания
    sound = spellSound;
    group = "spell";
  } else if (panelNumber >= 2 && panelNumber <= 5) {
    // Кадры 2-5 — ночная атмосфера
    sound = comicNight;
    group = "night";
  } else if (panelNumber === 6) {
    // Кадр 6 — звук самолёта
    sound = finalPlaneSound;
    group = "plane";
  } else if (panelNumber === 7 || panelNumber === 9) {
    // Кадры 7 и 9 — шум аэропорта
    sound = finalAirportSound;
    group = "airport";
  } else if (panelNumber === 8) {
    // Кадр 8 — приглушённый шум аэропорта
    sound = finalAirportMuffledSound;
    group = "airport-muffled";
  } else if (panelNumber === 10 || panelNumber === 11) {
    // Кадры 10-11 — милая мелодия
  ...
  // Если группа не изменилась — ничего не делаем
  if (finalComicSoundGroup === group) {
    return;
  }
